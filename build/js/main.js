...
var dataObj = {
    ...
    data: [{
        ...
        answers: ['Скорость работы', 'Соответствие современным стандартам веб-разработки', 'Простота написания кода', 'Возможность обновить данные на странице не перезагружая ее'],
        right: [4]
    }, {
        ...
        answers: ['Синхронные запросы рекомендуются для GET запросов, асинхронные - при POST', 'Синхронный запрос выполняется быстрее', 'При синхронном запросе браузер “подвисает” до тех пор,' + 'пока не прийдет ответ с сервера, при асинхронном -' + 'браузер продолжает работать и ответ обрабатывается при помощи' + 'специальной функции-слушателя', 'Асинхронные запросы - новая технология,' + 'которая не поддерживается в старых браузерах.'],
        right: [3]
    }, {
        question: 'Какое поле объекта XMLHttpRequest используется' + 'для обработки полученного от сервера ответа?',
